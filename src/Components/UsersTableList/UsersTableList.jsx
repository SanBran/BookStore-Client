...
import { getTableUsers } from "../../redux/actions/actions.js";
...
const Listusers = () => {
    ...
    const openModal = (id_delete, title_delete) => {
                ...
                <Card>
                  <Table className={`mt-5 ${styles.tableContainer}`}>
                    ...
                    <TableBody className={styles.tableBody}>
                      {listUsersAdm?.map((item) => (
                          <TableRow key={item.id}>
                            {/* { console.log("item", item)} */}
                          <TableCell>
                            <p className={`${styles.boldText}`}>{item.name || "n/a"}</p>
                            {item.email}
                          </TableCell>
                          <TableCell>
                            <Text>{item.country  || "n/a"}</Text>
                          </TableCell>     
                          <TableCell>
                            <Text>{item.rol}</Text>
                          </TableCell>
      ...
      console.log("error user-tables-->", error.message);
    }
};
